...
for (file_id, package_id, file_name, metadata_type_id) in cursor:
    ...
    if file_name:
        ...
        elif 'omics' in file_name.lower(): # might be something better we can key off of here
            if 'lipidomics' in file_name.lower():
                omicsType = 'lipidomics'
            elif 'metabolomics' in file_name.lower():
                omicsType = 'metabolomics'
            elif 'n-glycomics' in file_name.lower():
                omicsType = 'n-glycomics'
            packageZipper.zip_package_data(package_id+'_'+omicsType, datalake_package_dir)
            update_file_size(file_path, file_id)
